// lib/SmartThings.js v2.1.8
const axios = require('axios');
const fs = require('fs').promises;
const path = require('path');
const { LRUCache } = require('lru-cache');
const axiosRetry = require('axios-retry'); // [개선] axios-retry 라이브러리 추가
...
class SmartThings {
    constructor(log, api, config) {
        ...
        // [개선] axios-retry를 사용하여 일반 API 요청 실패 시 자동 재시도 (3회)
        ...
        // [개선] 캐시 유효시간(ttl)을 2초에서 5초로 늘려 불필요한 API 호출 감소
        this.cache = new LRUCache({ max: 100, ttl: 1000 * 5 }); // 5초
    ...
    // [개선] 토큰 갱신 실패 시 2초 간격으로 최대 3회 재시도하여 안정성 강화
    async refreshToken() {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                this.log.info(`액세스 토큰 갱신을 시도합니다... (시도 ${attempt}/${MAX_RETRIES})`);
                ...
                await this.saveTokens(response.data);
                return this.tokens.access_token; // 성공 시 토큰 반환 및 함수 종료

            } catch (error) {
                this.log.error(`토큰 갱신 시도(${attempt}) 실패:`, error.message);
                if (attempt === MAX_RETRIES) {
                    throw error; // 마지막 시도도 실패하면 에러를 던짐
                }
                // 2초 대기 후 재시도
    ...
    // [개선] 파일 쓰기 오류에 대비해 try/catch로 감싸 안정성 강화
