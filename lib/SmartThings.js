// lib/SmartThings.js
// SmartThings API 연동, 승준 에어컨 기능 매핑 한글 커스텀 버전 (디버그 로그 강화)
...
class SmartThings {
  ...
  async getDevices() {
    try {
      ...
      this.log('[디버그] SmartThings API에서 전체 기기 label:', (res.data.items || []).map(x => `[${x.label}]`).join(', '));
  ...
  async getMode(deviceId) {
    try {
      ...
      this.log(`[디버그] [${deviceId}] 모드 status 응답:`, JSON.stringify(res.data));
      return res.data.airConditionerMode?.airConditionerMode?.value || 'off';
    } catch (e) {
      this.log('운전모드 조회 오류:', e.response?.data || e.message);
      return 'off';
    }
  }
