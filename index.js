...
      characteristic: Characteristic.CurrentHeaterCoolerState,
      // ▼▼▼ [수정 1/2] 현재 상태 표시 로직 변경 ▼▼▼
      getter: async () => {
        // 전원이 꺼져 있으면 '비활성'으로 표시합니다.
        if (!await this.smartthings.getPower(deviceId)) {
          return Characteristic.CurrentHeaterCoolerState.INACTIVE;
        }
        // 사용자의 요청에 따라, 전원이 켜져 있으면 실제 모드(냉방, 제습, 송풍 등)와 관계없이 항상 '냉방 중'으로 표시합니다.
...
      characteristic: Characteristic.TargetHeaterCoolerState,
      // ▼▼▼ [수정 2/2] 선택 가능 모드 제한 ▼▼▼
      // 사용자의 요청에 따라, 홈킷에서 선택 가능한 모드를 '냉방'으로 제한합니다. '난방'과 '자동'은 제거됩니다.
...
        switch (mode) {
          case 'cool':
          case 'dry': // '제습' 모드도 홈킷에서는 '냉방'으로 표시
            return Characteristic.TargetHeaterCoolerState.COOL;
          case 'heat':
            return Characteristic.TargetHeaterCoolerState.HEAT;
          default: // 'auto', 'wind' 등 다른 모든 모드
...
            case Characteristic.TargetHeaterCoolerState.COOL:
                // 사용자의 요청에 따라, 홈킷의 '냉방'은 실제 기기의 '제습' 모드로 설정합니다.
                mode = 'dry';
                break;
